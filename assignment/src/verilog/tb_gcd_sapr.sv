module tb_gcd #(
    parameter real CLK = 2.5,           // change this clk frequency according to the apr result (here is 2.5ns)
    parameter real OFFSET = 0.15,       // this is the INPUT_DELAY in .tcl for both reset signals and data signals
    ...
    supply1 VDD;                        // after apr, there will be vdd and vss from back-end
    supply0 VSS;
...
`include "tasks.sv"
    ...
    initial begin
        ...
        $fsdbDumpvars(0, DUT);
        `ifdef SDF 
            $sdf_annotate("./gcd.wc.sdf", DUT, "./sdf.max.cfg"); 
        `endif
